    ...
    >
      ...
      <div class="detailContent flex flex-col gap-1 w-full">
        ...
        <div class="detail1 flex items-center gap-2">
          ...
          <div class="name text-[15px] font-semibold">최정현</div>
        </div>
        ...
        <div class="detail3 flex items-center text-xs text-black-700 gap-1">
          ...
          <p class="truncate pt-1 text-[14px] text-orange-500 font-semibold">
            동대구역
          </p>
        </div>
      </div>
...
const buttonText = computed(() => {
  if (clickCount.value === 0) return "완료";
  if (clickCount.value === 1) return "완료";
  return "완료";
});

const buttonClass = computed(() => {
  if (clickCount.value === 0) return "bg-[#4F46E5]";
  return "bg-gray-400";
});
